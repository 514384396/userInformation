...
public class UserDao {
    ...
    public  int add(User user) {
        ...
        ResultSet rs = null;
        try {
            String sql = "insert into usermessage(grade, name, username, password, gender, age, address, qq, email,phone) values (?,?,?,?,?,?,?,?,?,?)";
            connection = DBUtil.getConnection();
            ps = connection.prepareStatement(sql);
    ...
    public  int updateUser(User upUser) {
        ...
        PreparedStatement ps = null;
        try {
            String sql = "update usermessage set grade=?,gender=?,age=?,address=?,qq=?,email=?,phone=? where id=?";
            connection = DBUtil.getConnection();
            ps = connection.prepareStatement(sql);
            ps.setInt(1,upUser.getGrade());
            ps.setString(2,upUser.getGender());
            ps.setInt(3,upUser.getAge());
            ps.setString(4,upUser.getAddress());
            ps.setString(5,upUser.getQq());
            ps.setString(6,upUser.getEmail());
            ps.setString(7,upUser.getPhone());
            ps.setInt(8,upUser.getId());
            int ret = ps.executeUpdate();
            return ret;
        }catch (SQLException e) {
            e.printStackTrace();
